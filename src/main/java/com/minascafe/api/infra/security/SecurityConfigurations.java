...
public class SecurityConfigurations {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity httpSecurity) throws Exception{
        return httpSecurity
                ...
                .sessionManagement(session -> session.sessionCreationPolicy(SessionCreationPolicy.STATELESS)) //autenticação stateless - via token. Ñ armazena infs. da sessão no servidor
                .authorizeHttpRequests(authorize -> authorize //Quais são as requisições http que serão autorizadas
                        .requestMatchers(HttpMethod.POST, "/auth/login").permitAll()  //permitindo que qualquer usuário faça requisições para tela de login
/*Posteriormente liberar somente para Admins*/ .requestMatchers(HttpMethod.POST, "/auth/register").permitAll() //Só para testar. Deve ser bloqueado. Permissão só para Admins
                        .requestMatchers(HttpMethod.POST, "/cafecoco").hasRole("ADMIN") //Qualquer request do tipo "POST" p/ /cafecoco quero q o usuário tenha role tipo "ADMIN"
                        .anyRequest().authenticated() //para todas as outras requisições o usuário deve estar autenticado apenas
                )
    ...
    public AuthenticationManager authenticationManager(AuthenticationConfiguration authenticationConfiguration) throws Exception {
        return authenticationConfiguration.getAuthenticationManager(); //Para pegar a instância de um authenticationManager do SpringSecurity
